    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                <span>
                    {{translate('Update Group Customer')}}
                </span>
            </h1>
        </div>
        <!-- End Page Header -->
        <div class="card">
            <div class="card-body">
                <form action="{{route('admin.group-customers.update',[$groupCustomer['id']])}}" method="post" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="row">
                        <div class="col-6">
                            @if (isset($language))
                                ...
                                @foreach($language as $lang)
                                    <?php
                                        if(count($groupCustomer['translations'])){
                                            $translate = [];
                                            foreach($groupCustomer['translations'] as $t)
                                            {
                            ...
                            @else
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" class="form-control" placeholder="{{translate('messages.new_brand')}}" value="{{$groupCustomer['name']}}" maxlength="191">
                                </div>
                                {{-- <input type="hidden" name="lang[]" value="{{$lang}}"> --}}
                            @endif
                        </div>
                        ...
                        <div class="col-6">
                            ...
                            <select class="form-control" id="business_owner_id" name="business_owner_id" required>
                                ...
                                <option value="{{ $owner->id }}" {{ $groupCustomer->business_owner_id == $owner->id ? 'selected' : '' }}>
                                    {{ $owner->name }}
                                </option>
                                @endforeach
                            </select>
