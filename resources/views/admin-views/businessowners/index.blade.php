    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                <span>
                    {{ translate('Bussiness Owner Setup	') }}
                </span>
            </h1>
        </div>
        <!-- End Page Header -->
        <div class="card">
            <div class="card-body">
                <form action="{{ route('admin.businessowners.store') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                            ...
                            @else
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name" class="form-control"
                                        placeholder="{{ translate('Bussiness Owner Name') }}" value="{{ old('name') }}"
                                        maxlength="191">
                                </div>
                                <input type="hidden" name="lang[]" value="default">
                            @endif
            ...
            <div class="card-body p-0">
                <div class="table-responsive datatable-custom">
                    <table id="columnSearchDatatable"
                        ...
                        @endphp
                        <tbody id="table-div">
                            @foreach ($businessOwners as $key => $businessOwner)
                                <tr>
                                    <td> {{ $i++ }}</td>
                                    <td>
                                        <span class="d-block font-size-sm text-body">
                                            {{ Str::limit($businessOwner['name'], 20, '...') }}
                                        </span>
                                    </td>
                                    <td>
                                        <div class="btn--container justify-content-center">
                                            <a class="btn action-btn btn--primary btn-outline-primary"
                                                href="{{ route('admin.businessowners.edit', [$businessOwner['id']]) }}"
                                               ><i class="tio-edit"></i>
                                            </a>
                                            <a class="btn action-btn btn--danger btn-outline-danger form-alert"
                                                href="javascript:" data-id="brand-{{ $businessOwner['id'] }}"
                                                data-message="{{ translate('messages.Want to delete this brand') }}"
                                                title="{{ translate('messages.delete_brand') }}"><i
                                                    class="tio-delete-outlined"></i>
                                            </a>
                                            <form action="{{ route('admin.businessowners.destroy', $businessOwner->id) }}" method="POST" id="brand-{{ $businessOwner->id }}">
                                                @csrf
                                                @method('DELETE')
